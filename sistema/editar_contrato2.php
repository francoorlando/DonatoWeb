<?php 
	...
	{
		$alert='';
		if(empty($_POST['cliente']) || empty($_POST['cpasajeros']) || empty($_POST['nhabitaciones']) || empty($_POST['importe']) ||empty($_POST['cab'])|| empty($_POST['tipodepago']) || empty($_POST['fechaingreso']) || empty($_POST['fechasalida']))
		{
		$alert='<center><strong><p class="alert-warning">Todos los campos son obligatorios.</p></strong></center>';
			...
			$usuario_id = $_SESSION['idUser'];
				...
				if($query_insert){
					   $alert='<center><strong><p class="alert-success">Contrato actualizado correctamente.</p></strong></center>';
				}else{
					$alert='<center><strong><p class="alert-danger">Error al guardar el contrato.</p></strong></center>';
				}
	...
	if(empty($_REQUEST['id'])){
		...
		$query_contrato = mysqli_query($conection,"SELECT c.codcontrato, c.descripcion, c.importe,c.cab, cl.nombre , c.cantpasajeros, c.nhabitaciones, c.tipodepago , c.fechaingreso, c.fechasalida FROM contrato c INNER JOIN cliente cl 
																	ON c.cliente = cl.idcliente 
																	WHERE c.codcontrato = $id_contrato AND c.estatus = 1 ");
 ...
 <li><a class="sidebar-sub-toggle"><i class="ti-calendar"></i>Reservas <span class="badge badge-primary"></span><span class="sidebar-collapse-icon ti-angle-down"></span></a>
  ...
  <li><a class="sidebar-sub-toggle"><i class="ti-stamp"></i>Asistencia Empleados <span class="badge badge-primary"></span><span class="sidebar-collapse-icon ti-angle-down"></span></a>
    ...
    <div class="content-wrap">
                        ...
                        <div class="page-header">
                            <div class="page-title">
                                <ol class="breadcrumb text-right">
                                    ...
                                    <li class="active">Actualizar Contrato</li>
                                </ol>
                            </div>
                        </div>
                    </div>
        ...
        <a href="lista_contrato2.php" class="ti-back-left " > Volver</a>
  
        </div>
			<h1>Actualizar Contrato</h1>
			<hr>
			...
			<form action="" method="post">
                ...
                <option><?php echo $data_contrato['tipodepago'];?></option>  
				...
				<input type="submit" value="Actualizar contrato" class="btn btn-primary btn-rounded m-b-10 m-l-5">
